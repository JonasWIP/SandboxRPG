...
public partial class BuildSystem : Node
{
	...
	public static bool IsBuildable(string? itemType) =>
		itemType != null && (StructureRegistry.Get(itemType)?.IsPlaceable ?? false);
	...
	private void CreateGhostPreview(string structureType)
	{
		...
		var def = StructureRegistry.Get(structureType);
		...
		else if (def is not null && !string.IsNullOrEmpty(def.ScenePath) && ResourceLoader.Exists(def.ScenePath))
		{
			var model = ResourceLoader.Load<PackedScene>(def.ScenePath).Instantiate<Node3D>();
			ApplyGhostMaterial(model);
			_ghostPreview.AddChild(model);
		}
		else
		{
			...
			{
				Mesh             = new BoxMesh { Size = def?.CollisionSize ?? Vector3.One },
			...
			mesh.Position = new Vector3(0, def?.YOffset ?? 0.5f, 0);
